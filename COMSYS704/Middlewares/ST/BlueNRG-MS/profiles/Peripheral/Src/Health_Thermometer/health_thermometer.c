/**
  ******************************************************************************
  * @file    health_thermometer.c
  * @author  AMS - HEA&RF BU
  * @brief   Source code for the sensor role of health thermometer profile
  ******************************************************************************
  * @attention
  *
...
tBleStatus HT_Add_Services_Characteristics(void)
{
  ...
  if(thermometer.thermometerFeatures & MEASUREMENT_INTERVAL_CHAR)
  {
    ...
    if (hciCmdResult == BLE_STATUS_SUCCESS)
    {
      HT_DBG_MSG (profiledbgfile, "HT_Add_Services_Characteristics(),Health Thermometer measurement interval Characteristic Added Successfully: %04X, buf: %02x%02x%02x%02x\n", thermometer.miCharHandle, buf[0], buf[1], buf[2],buf[3]);
